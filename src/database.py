...
def update_event_info(records):
    ...
    try:
        cursor.executemany(
            ...
            SET grade = ?, kubun = ?, nichiji = ?
            WHERE event_date = ? AND venue_name = ?
            """,
            [
                (
                    r["grade"],
...
def save_generate_history(history_type, start_date, end_date):
    conn = get_connection()
    cursor = conn.cursor()
    ...
    cursor.execute(
        ...
        VALUES (?, ?)
        """,
        (f"{history_type}_from", str(start_date)),
    )
    ...
    cursor.execute(
        ...
        VALUES (?, ?)
        """,
        (f"{history_type}_to", str(end_date)),
    )
    conn.commit()
    conn.close()
...
def get_generate_history(history_type):
    conn = get_connection()
    cursor = conn.cursor()
