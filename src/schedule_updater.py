...
def update_schedule_info():
    ...
    # KEIRIN.JP取得
    # -----------------------------
    schedule = get_schedule(sorted(months))

    race_map = {}

    for race in schedule["RaceList"]:
        key = (
            datetime.strptime(race["kaisaiDate"], "%Y%m%d").strftime("%Y-%m-%d"),
    ...
    for event_date, venue, *_ in events:
        ...
        update_records.append({
            "date": event_date,
            "venue": venue,
            "grade": race.get("gradeIconName"),
            "kubun": race.get("kubunIconName"),
            "nichiji": race.get("nichijiIconName"),
        })
